...
| `components/link.tsx` | App-facing `Link` that wraps the in-repo `Link`. It owns one non-transition behavior: a link to the page you are already on (`isSamePagePath`, no hash) smooth-scrolls to top via native `window.scrollTo`. Every transition concern (cross-page navigation, same-page hash, modal `params`) is handled inside the wrapped `Link`. |
